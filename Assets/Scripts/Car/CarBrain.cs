...
public class CarBrain : MonoBehaviour
{
	...
	private void Awake()
	{
		Network = NeuralNetwork.Random(new NeuralNetworkSettings(InputNeuronsCount, 6, 6, OutputNeuronsCount));
	}
	...
	private void UpdateNetwork()
	{
		...
		Debug.Log($"Wheel (Front Left): {result[0]}; Wheel (Rear Left): {result[1]};  Wheel (Front Right): {result[2]};  Wheel (Rear Right): {result[3]}");
